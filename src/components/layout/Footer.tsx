...
const columns = [
  ...
  },
  {
    ...
    links: [
      ...
      { href: "mailto:info@afralawyersclub.ir", label: "info@afralawyersclub.ir" },
    ],
  },
];
  ...
  return (
    <footer className="border-t border-neutral-100 bg-neutral-900 text-neutral-300">
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 px-4 py-12 sm:grid-cols-2 sm:px-6 lg:grid-cols-4 lg:px-8">
        ...
        {columns.map((col) => (
          <div key={col.title}>
            ...
            <ul className="mt-4 flex flex-col gap-2.5">
              {col.links.map((l) => (
                <li key={l.href}>
                  <Link href={l.href} className="text-sm text-neutral-400 transition-fast hover:text-white">
                    {l.label}
                  </Link>
                </li>
              ))}
        ...
        <div>
          <p className="text-sm font-semibold text-white">شبکه‌های اجتماعی</p>
          <ul className="mt-4 flex gap-3 text-sm text-neutral-400">
            <li>اینستاگرام</li>
            <li>لینکدین</li>
          </ul>
        </div>
